...
% h1 = fill([x fliplr(x)], [mean_max_sq+hw_max_sq fliplr(mean_max_sq-hw_max_sq)], [0 0.4470 0.7410], 'EdgeColor','none', 'FaceAlpha',0.3);
% h1.HandleVisibility = 'off';
% h2 = fill([x fliplr(x)], [mean_lyap_sq+hw_lyap_sq fliplr(mean_lyap_sq-hw_lyap_sq)], [0.8500 0.3250 0.0980], 'EdgeColor','none', 'FaceAlpha',0.3);
% h2.HandleVisibility = 'off';
% 
% % Plot performance lines with DisplayName for legend
% plot(x, mean_max_sq, 'b-', 'LineWidth', 1.5, 'DisplayName', 'MaxWeight');
% plot(x, mean_lyap_sq, 'r-', 'LineWidth', 1.5, 'DisplayName', 'LyapOpt');
% 
% xlabel('Time slot T');
% ylabel('Sum of squares of queue lengths');
% title('Sum of squares of queue lengths performance');
